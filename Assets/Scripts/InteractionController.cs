...
{
	/***** Public Variables *****/
  // List of cubes which can be highlighted and clicked via the controller.
  public GameObject[] bubbleCursorItems;

  // The controller used to trigger cube interactions. 
  public ActionBasedController XRController;

  // The maximum distance to perform a Raycast
  ...
  private void Start() {
    // Subscribe to the input event when the trigger is pressed
    XRController.selectAction.action.performed += OnTriggerPress;

    // Get the XRRayInteractor component from the controller used to trigger
    // cube interactions.
    _XRControllerRayInteractor = XRController.GetComponentInChildren<XRRayInteractor>();
  }

  private void OnDestroy() {
    // Unsubscribe from the input event when the script is destroyed
    XRController.activateAction.action.performed -= OnTriggerPress;
  }

  private void Update() {
    // Highlight the nearest cube if the game has started
    if (GameController.isGameStarted) HighlightNearestCube();
  }

  /** Using the `xrControllerRayInteractor`, highlight the nearest cube */
